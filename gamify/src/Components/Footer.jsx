...
const Footer = ({ isDashboard = false }) => {
  ...
  return (
    <footer className={`${isDashboard ? 'bg-gray-50 border-t border-gray-200 mt-8' : 'bg-slate-900 text-white mt-16'} relative`}>
      <div className={`container relative z-10 ${isDashboard ? 'py-8 px-4 sm:px-6 lg:px-8' : 'py-12 pl-4 sm:pl-6 lg:pl-8'}`}>
        {/* Top Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 mb-10">
          {/* Logo + About */}
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center gap-3">
              <div className={`${isDashboard ? 'bg-gray-800' : 'bg-blue-600'} w-10 h-10 rounded-xl flex items-center justify-center shadow-lg`}>
                <span className="text-white text-lg font-bold">Q</span>
              </div>
              <span className={`text-2xl font-bold ${isDashboard ? 'text-gray-800' : 'text-white'}`}>
                Q&A Platform
              </span>
            </div>
            <p className={`${isDashboard ? 'text-gray-600' : 'text-gray-300'} leading-relaxed max-w-md text-base`}>
              Connect with experts, join interactive Q&A sessions, and earn rewards while learning in a vibrant global community.
            </p>

            {/* Social Links - Only show in non-dashboard mode */}
            {!isDashboard && (
          ...
          {/* Platform - Simplified for dashboard */}
          {isDashboard ? (
            <div className="space-y-4">
              <h4 className="font-semibold text-gray-800 text-base uppercase tracking-wider">Quick Links</h4>
              <nav className="flex flex-col space-y-3">
                <a href="/explore" className="text-gray-600 hover:text-blue-600 transition-all text-base hover:translate-x-1">Browse Sessions</a>
                <a href="/leaderboards" className="text-gray-600 hover:text-blue-600 transition-all text-base hover:translate-x-1">Leaderboard</a>
                <a href="/my-sessions" className="text-gray-600 hover:text-blue-600 transition-all text-base hover:translate-x-1">My Sessions</a>
                <a href="/profile" className="text-gray-600 hover:text-blue-600 transition-all text-base hover:translate-x-1">Profile</a>
              </nav>
            </div>
          ) : (
            <>
        ...
        {/* Newsletter Signup - Only show in non-dashboard mode */}
        {!isDashboard && (
