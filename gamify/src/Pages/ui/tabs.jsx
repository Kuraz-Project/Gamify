...
export const TabsList = ({children, className = '', active, setActive}) => (
  <div className={`inline-grid gap-2 rounded-md bg-gray-150 p-1  ${className}`}>{React.Children.map(children, child => React.isValidElement(child) ? React.cloneElement(child, {active, setActive}) : child)}</div>
)

export const TabsTrigger = ({value, children, active, setActive, className = ''}) => (
  <button onClick={() => setActive(value)} className={`px-3 py-2 rounded text-sm ${active === value ? 'bg-white ' : 'text-gray-600 dark:text-gray-300'} ${className}`}>{children}</button>
)
